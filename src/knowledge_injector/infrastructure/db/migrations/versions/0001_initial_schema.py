...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "source_id",
            UUID(as_uuid=True),
            sa.ForeignKey(f"{SCHEMA}.knowledge_sources.id", ondelete="CASCADE"),
            nullable=False,
        ),
    ...
    op.create_index(
        "ix_ingestion_runs_source_id", "ingestion_runs", ["source_id"], schema=SCHEMA
    )
    op.create_index(
        "ix_ingestion_runs_status", "ingestion_runs", ["status"], schema=SCHEMA
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "mime_type", sa.String(100), nullable=False, server_default="text/plain"
        ),
    ...
    op.create_index(
        "ix_knowledge_documents_source_path",
        "knowledge_documents",
        ["source_id", "path"],
        unique=True,
        schema=SCHEMA,
    ...
    op.create_table(
        ...
        sa.Column(
            "document_id",
            UUID(as_uuid=True),
            sa.ForeignKey(f"{SCHEMA}.knowledge_documents.id", ondelete="CASCADE"),
            nullable=False,
        ),
        ...
        ),  # placeholder — replaced below with vector(768)
    ...
    op.execute(
        f"ALTER TABLE {SCHEMA}.knowledge_chunks ADD COLUMN embedding vector(384) NULL"
    )
